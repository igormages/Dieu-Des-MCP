...
let memorySession: KrakenSessionState | null = null;
let inflightLogin: Promise<KrakenSessionState> | null = null;

async function getKrakenCredentials(): Promise<KrakenCredentials> {
...
function parseGraphqlBody<T>(text: string): T {
  ...
  if (body.errors?.length) {
    throw new Error(`Kraken GraphQL: ${body.errors.map((e) => e.message).join("; ")}`);
  }
  if (!body.data) {
    throw new Error("Kraken GraphQL: réponse vide.");
  }
  return body.data;
}
...
async function loadSession(): Promise<KrakenSessionState | null> {
  ...
  const raw = await kv.get<KrakenSessionState>(SESSION_KEY);
  if (!raw || raw.tokenExpiresAt <= Date.now() + 60_000) return null;
  memorySession = raw;
  return raw;
}

async function saveSession(state: KrakenSessionState): Promise<void> {
  memorySession = state;
  const kv = getKvClient();
  if (!kv) return;
...
export async function octopusKrakenClearSession(): Promise<void> {
  memorySession = null;
  const kv = getKvClient();
  if (kv) await kv.del(SESSION_KEY);
}

async function krakenGraphqlRequest<T>(
  token: string | null,
  query: string,
  ...
  if (!response.ok) {
    throw new Error(`Kraken API ${response.status}: ${text.slice(0, 300)}`);
  }
  return parseGraphqlBody<T>(text);
}

async function performKrakenLogin(): Promise<KrakenSessionState> {
  const creds = await getKrakenCredentials();

  const loginData = await krakenGraphqlRequest<{
    obtainKrakenToken?: {
      token: string;
      refreshToken: string;
      refreshExpiresIn: number;
  ...
  if (!loginResult?.token) {
    throw new Error("Kraken login : token absent dans la réponse.");
  }

  let refreshToken = loginResult.refreshToken;
  let refreshExpiresIn = loginResult.refreshExpiresIn;

  const longLived = await krakenGraphqlRequest<{
  ...
  const state: KrakenSessionState = {
...
async function ensureKrakenSession(): Promise<KrakenSessionState> {
  let state = await loadSession();
  if (!state) {
    if (!inflightLogin) inflightLogin = performKrakenLogin();
    try {
  ...
  if (state.tokenExpiresAt <= Date.now() + 60_000) {
    ...
    state = await performKrakenLogin();
  }
  return state;
}

async function resolveAccountNumber(state: KrakenSessionState): Promise<string> {
  ...
  if (!accountNumber) {
    throw new Error("Aucun compte Octopus Energy trouvé via l'API Kraken.");
  }
...
export async function octopusKrakenGetSessionStatus(): Promise<OctopusKrakenSessionStatus> {
  const creds = await getKrakenCredentials();
...
export async function octopusKrakenForceRelogin(): Promise<OctopusKrakenSessionStatus> {
  await octopusKrakenClearSession();
  await performKrakenLogin();
  return octopusKrakenGetSessionStatus();
}
